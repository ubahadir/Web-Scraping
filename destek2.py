...
for contact in unread_contacts:
    ...
    if "Contact" in contact:
        try:
            ...
            #contact number son kısmı div/span fakat farklı durumlarda bulamayabilir
    ...
    if group == False:
        ...
        WebDriverWait(driver, 5, ignored_exceptions=ignored_exceptions).until(
            EC.visibility_of_all_elements_located((By.XPATH, '//*[@id="main"]/div[2]/div/div[2]/div/div'))
        )
        ...
        for row in all_rows:
            ...
            #DÜZ MESAJI EKLİYOR            
                ...
                try:
                    message = row.find_element(By.XPATH, ".//*[@class='_21Ahp']").text
            ...
            # whatsapptaki gereksiz rowları ayıklıyor
                ...
                pass
            # her mesajın divinde zaten focusable-list-item olduğu için bu yöntem yanlış
            ...
            # try bloğunu kaldırıp hataları bulman gerekiyor.
                try:
                    msg_date = row.find_element(
                        By.XPATH, './/*[@data-pre-plain-text]').get_attribute('data-pre-plain-text')
                    msg_dates.append(msg_date)
                #neden no date ekliyorsun? pass desen senkronize olmaz mı?
                except NoSuchElementException:
                    msg_dates.append("no date")
